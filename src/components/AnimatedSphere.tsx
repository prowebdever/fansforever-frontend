...
const AnimatedSphereWrapper = styled.div`
  position: fixed;
  height: 100vh;
  width: 100%;
  top: 0;
...
const AnimatedSphere: React.VFC = () => {
  const [spherePathEls, setSpherePathEls] = useState<any>(null);
  ...
  useEffect(() => {
    if (sphereElRef?.current) {
      ...
      const spherePathElements = sphereEl?.querySelectorAll('.sphere path');
      setSpherePathEls(spherePathElements);
      setPathLength(spherePathElements?.length || 0);
    }
  }, [sphereElRef]);
  ...
  const breathAnimation = useMemo(
    ...
    [pathLength, spherePathEls, animations]
  );
  ...
  useEffect(() => {
    if (introAnimation && breathAnimation && shadowAnimation) {
      introAnimation.play();
      breathAnimation.play();
      shadowAnimation.play();
    }
  }, [introAnimation, breathAnimation, shadowAnimation]);
